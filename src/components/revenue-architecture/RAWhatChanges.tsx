"use client";

import { useRef } from "react";
import { motion, useInView } from "framer-motion";

const outcomes = [
  {
    title: "Pipeline becomes visible.",
    body: "Leadership sees what\u2019s real, what\u2019s at risk, and what\u2019s moving \u2014 in real time.",
  },
  {
    title: "Execution becomes consistent.",
    body: "Every rep follows the same motion. Growth stops depending on who\u2019s having a good month.",
  },
  {
    title: "The system compounds.",
    body: "Good architecture gets better over time. Data improves. Workflows get tighter. And it becomes the foundation for AI when you\u2019re ready.",
  },
];

export default function RAWhatChanges() {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-80px" });

  return (
    <section className="relative overflow-hidden bg-[var(--off-white)] pb-0 pt-20 lg:pt-28">
      <div ref={ref} className="mx-auto max-w-[1200px] px-6 lg:px-8">
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          {/* Left — image */}
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            animate={isInView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.9, delay: 0.2 }}
            className="relative order-2 lg:order-1"
          >
            <div className="overflow-hidden rounded-2xl border border-[var(--light-gray)] bg-white shadow-md">
              <div className="aspect-[4/3] w-full">
                <img
                  src="/images/ra-outcomes.jpg"
                  alt="Revenue dashboard showing pipeline visibility"
                  className="h-full w-full object-cover"
                  onError={(e) => {
                    const target = e.target as HTMLImageElement;
                    target.style.display = "none";
                    target.parentElement!.classList.add("flex", "items-center", "justify-center");
                    const placeholder = document.createElement("div");
                    placeholder.className = "text-center p-8";
                    placeholder.innerHTML = '<p class="text-sm text-[var(--graphite)]/40">Outcomes image placeholder</p><p class="mt-1 text-xs text-[var(--graphite)]/30">Recommended: 800×600</p>';
                    target.parentElement!.appendChild(placeholder);
                  }}
                />
              </div>
            </div>
          </motion.div>

          {/* Right — outcomes */}
          <div className="order-1 lg:order-2">
            <motion.p
              initial={{ opacity: 0, x: -20 }}
              animate={isInView ? { opacity: 1, x: 0 } : {}}
              transition={{ duration: 0.6 }}
              className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--brand-red)]"
            >
              The Outcomes
            </motion.p>
            <motion.h2
              initial={{ opacity: 0, y: 40 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.8, delay: 0.1 }}
              className="mt-6 text-[clamp(2rem,4vw,3rem)] font-bold leading-[1.1] tracking-tight text-[var(--dark-graphite)]"
            >
              What changes when
              <br />
              the architecture is in place.
            </motion.h2>
            <motion.div
              initial={{ scaleX: 0 }}
              animate={isInView ? { scaleX: 1 } : {}}
              transition={{ duration: 0.8, delay: 0.3 }}
              className="mt-6 h-[2px] w-16 origin-left bg-[var(--brand-red)] opacity-40"
            />

            <div className="mt-10 space-y-8">
              {outcomes.map((outcome, i) => (
                <motion.div
                  key={outcome.title}
                  initial={{ opacity: 0, y: 24 }}
                  animate={isInView ? { opacity: 1, y: 0 } : {}}
                  transition={{ duration: 0.6, delay: 0.3 + i * 0.12 }}
                >
                  <h3 className="text-lg font-bold text-[var(--dark-graphite)]">
                    {outcome.title}
                  </h3>
                  <p className="mt-2 text-base leading-relaxed text-[var(--graphite)]">
                    {outcome.body}
                  </p>
                </motion.div>
              ))}
            </div>
          </div>
        </div>
      </div>

      {/* Bottom curve into white */}
      <div className="relative -mb-px mt-16 lg:mt-24">
        <svg
          className="block w-full"
          viewBox="0 0 1440 80"
          fill="none"
          preserveAspectRatio="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M0 80V40C360 0 720 0 1080 20C1260 30 1380 45 1440 55V80H0Z"
            fill="white"
          />
        </svg>
      </div>
    </section>
  );
}
